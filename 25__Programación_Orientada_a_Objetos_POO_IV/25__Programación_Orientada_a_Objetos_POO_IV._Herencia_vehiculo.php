...
/// Una clase puede tener sus propias constantes, variables(llamadas "propiedades"), y funciones (llamados "métodos"). 
{
   ...
   /**
    * Este es un método constructor en PHP que inicializa las propiedades de una clase.
    */
...
/**
 * Esta función de PHP crea un color para un automóvil.
 * 
 * @param $color_coche color_coche es un parámetro que representa el color de un coche. Se usa en la
 * función crear_color() para establecer la propiedad de color de un objeto al valor del parámetro
 * color_coche.
 */
...
class Camion extends Coche // Coche (Super clase ó clase Padre), Camion(Sub-clase ó clase Hijo )
{
    ...
    function arrancar(){
      /* `parent::arrancar();` está llamando al método `arrancar()` de la clase principal (Coche) desde
      dentro del método `arrancar()` de la clase secundaria (Camion). Esto permite que la clase
      secundaria herede y amplíe la funcionalidad de la clase principal. */
